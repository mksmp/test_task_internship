# Мониторинг времени up-time веб-приложений банков и их мобильных приложений 

### Disclaimer - автор данного репозитория не несет ответственность за использование данных скриптов, данного приложения. Автор не хотел нанести какой-либо вред сервисам, которых он выбрал для отправки GET-запросов. Автор не является клиентом данных банков, поэтому не мог авторизироваться в них. GET-запросы отправлялись только на сайты банков в целях проверки включен или нет в данный момент сервис.
...
+ В настроках сети WIFI смартфона перенаправил трафик через ноутбук.
...
+ На смартфоне открывал приложения банков, не авторизируясь в них, и смотрел на какие dns идут запросы.
+ Проблема - некоторые банки для аутентификации используют сервера Google (на мой взгляд) и не понятно как оценивать состояние сервера, к которому обращается приложение.
